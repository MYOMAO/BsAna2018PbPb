		...
		Float_t lumi, Float_t raa=1.)
{
	...
	Double_t effS[NEff], effB[NEff], effSig[NEff], SB[NEff]; 
	int n=0;
	//	cout << "Pass 4" << endl;
		cout << "eff = " << eff << endl;
	while(eff)
	...
	//
	calRatio(signal, background, generated, cuts, cutb, cutg, weightfunctiongen, weightfunctionreco, ptmin, ptmax, FONLLName.Data(), lumi, raa, outputname, weights);
	//
...
/*
	for(int i=0;i<NEff;i++)
	{
		...
		SB[i] = wSignal*effS[i]/(wBackground*effB[i]);
		cout << "Signal Efficiency =  " << effS[i] << "   Signal = " << wSignal*effS[i] << "   Background = " << wBackground*effB[i] << endl;
		if(effSig[i]>maxsig)
		{
			maxsig=effSig[i];
			maxindex=i;
...
int main(int argc, char* argv[])
{
	if(argc==16)
	{
		readxml_Cuts(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], argv[10], argv[11], atof(argv[12]), atof(argv[13]), atof(argv[14]), atof(argv[15]));
		return 0;
	}
	if(argc==15)
	{
		readxml_Cuts(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], argv[10], argv[11], atof(argv[12]), atof(argv[13]), atof(argv[14]));
		return 0;
	}
	else
	{
